...
import type { LookalikeRow } from "@/data/mockReports";
...
export const LookalikesTable: React.FC<LookalikesTableProps> = ({ data }) => {
  ...
  const handleRowClick = (row: LookalikeRow) => {
    navigate('/reports/lookalikes/detail', { state: { source: 'lookalike', row } });
  };
          ...
          <TableBody>
            {data.map((row) => (
              ...
              >
                ...
                <TableCell className="text-slate-300">{row.urlConstruction}</TableCell>
